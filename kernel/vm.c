...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    // ** extract new flags from updated pte.
    flags = PTE_FLAGS(*pte);

    // ** Disable alloc new memory for child
    // if((mem = kalloc()) == 0)
    //   goto err;
...
int checkcowpage(uint64 va, pte_t *pte, struct proc* p) {
  return (va < p->sz) // va should smaller the size of process memory (bytes)
         && (*pte & PTE_V)
         && (*pte & PTE_C); // pte is COW page
}
