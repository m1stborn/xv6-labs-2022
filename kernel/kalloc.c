...
int ref_cnt[PHYSTOP/PGSIZE]; // maximum number of page = PHYSTOP(byte, 128MB)/PGSIZE
...
kfree(void *pa)
{
  ...
  // ** Decrease the reference count if the use reference is not zero, then return.
  acquire(&ref_cnt_lock);
  ref_cnt[(uint64)pa/PGSIZE] -= 1;
  tmp = use_refs[(uint64)pa/PGSIZE];
